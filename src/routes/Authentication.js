...
router.post("/login", fetchMember, async (req, res) => {
  ...
  if (!validatedPassword) {
    return res.status(401).json({
      error: true,
      context:
        "You provided a set of invalid credentials while trying to authenticate your account.",
    });
  }
...
router.post("/", async (req, res) => {
  ...
  const username = req.body.username.toLowerCase(),
    ...
    birthdate = new Date(
      segmentedBirthdate[0],
      segmentedBirthdate[1] - 1,
      segmentedBirthdate[2]
    );
  ...
  if (age < 13) {
    return res.status(406).json({
      error: true,
      context:
        "You must be 13 years or older to use this service due to COPPA.",
    });
  }
  ...
  if (
    ...
    return res.status(406).json({
      error: true,
      context:
        "The credentials provided did not meet the criteria required for the account fields.",
    });
  }
  ...
  try {
    ...
    if (member === null || member === undefined) {
      return res.status(404).json({
        error: true,
        context:
          "We could not find an existing account using the provided Email Address!",
      });
    }
  } catch (error) {
    return res.status(500).json({
